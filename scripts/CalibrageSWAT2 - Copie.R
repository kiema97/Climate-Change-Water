...
Q_Obs_Data <- dbReadTable(conn = DataBase,name ="QObsData" )%>%
  ...
  filter(DATE>=as.Date("1983-01-01"),DATE<=as.Date("1992-12-31"),!is.na(Debit))

CalibPeriode <- Q_Obs_Data$DATE
...
SWATParametres <- readRDS(file = file.path("DataBase","SWATPARAMETERS.rds"))%>%
  ...
  slice(1)
##--------------------SWAT Modèle Run-----------------------------------------------------
SWATParametres <- readRDS(file = "D:/IGEDD/Memoire/Projet_Nowkuy/OUTPUT/Data/SWAT_PARAMETERSS.rds")%>%
  slice(1)%>%
  select(-nse)
SWATProject <- dir("D:/IGEDD/Memoire/Projet_Nowkuy/SWATProject")[-c(1:4)]
...
QOBS <- dbReadTable(conn = DataBase,name ="QObsData" )%>%
  mutate(DATE=as.Date(DATE))
calib.periode <- lubridate::interval(start =as.Date("2003-01-01") ,
                                          end = as.Date("2008-12-31"))

valid.periode <- lubridate::interval(start =as.Date("2009-01-01") ,
...
dbWriteTable(conn = DataBase,name ="SWATQSim",value = SWATQSim, field.types=c(DATE="DATE") )
